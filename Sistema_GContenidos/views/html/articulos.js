...
function cargarArticulos() {
    ...
    xhr.open("GET", "../controllers/articulos.controllers.php", true);
...
document.getElementById("formArticulo").onsubmit = function(event) {
    ...
    var xhr = new XMLHttpRequest();
    if (id_articulo) {
        xhr.open("PUT", "../controllers/articulos.controllers.php", true);
    } else {
        xhr.open("POST", "../controllers/articulos.controllers.php", true);
    }
...
function eliminarArticulo(id_articulo) {
    if (confirm("¿Está seguro de eliminar este artículo?")) {
        var xhr = new XMLHttpRequest();
        xhr.open("DELETE", "../controllers/articulos.controllers.php", true);
        xhr.setRequestHeader("Content-Type", "application/json");
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                cargarArticulos();
