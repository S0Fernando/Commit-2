...
function cargarEtiquetas() {
    ...
    xhr.open("GET", "../controllers/etiquetas.controllers.php", true);
...
document.getElementById("formEtiqueta").onsubmit = function(event) {
    ...
    var xhr = new XMLHttpRequest();
    if (id_etiqueta) {
        xhr.open("PUT", "../controllers/etiquetas.controllers.php", true);
    } else {
        xhr.open("POST", "../controllers/etiquetas.controllers.php", true);
    }
...
function eliminarEtiqueta(id_etiqueta) {
    if (confirm("¿Está seguro de eliminar esta etiqueta?")) {
        var xhr = new XMLHttpRequest();
        xhr.open("DELETE", "../controllers/etiquetas.controllers.php", true);
        xhr.setRequestHeader("Content-Type", "application/json");
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                cargarEtiquetas();
